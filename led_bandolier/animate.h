...
byte brightnessPulse(uint32_t time) {
  // Repeat pulse every 5 seconds.
  int PERIOD_MILLIS = 5000;
  int HALF_PERIOD = PERIOD_MILLIS / 2;
  int QTR_PERIOD = PERIOD_MILLIS / 4;
  int period_t = time % PERIOD_MILLIS;
  
  if (period_t < HALF_PERIOD) {
    return MAX_BRIGHT;
  } else if (period_t < HALF_PERIOD + QTR_PERIOD) {
...
void animate(void (*fn)(AnimationState*, byte[])) {
  ...
  if (otherNodesCount > 0) {
    ...
    brightness = brightnessPulse(state.time);
  }
